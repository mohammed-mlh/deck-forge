...
function CardDetailContent({ card }: { card: YugiohCard }) {
  ...
  return (
    <>
      ...
      {hasStats && (
        <div className="mb-4 grid grid-cols-2 gap-3 rounded-lg border border-(--color-border) bg-(--color-bg-elevated) p-4">
          {card.atk !== undefined && (
            <div>
  ...
  return (
    <>
      ...
      <>
        ...
        <aside
          className={cn(
            "fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-(--color-border) bg-(--color-bg-surface) shadow-md lg:hidden",
            className
          )}
        >
